...
/// Guard the feature-buffer length before handing a *borrowed* buffer to
/// ONNX Runtime.
///
/// The inference path used to build an owned `Array3` via
/// `from_shape_vec`, which validated the shape as a side effect of the
/// copy. Now that the buffer is passed by reference, the check has to be
/// explicit.
fn check_feature_len(features: &[f32], expected: usize) -> Result<(), EmbeddingError> {
    if features.len() == expected {
        Ok(())
    } else {
        Err(EmbeddingError::Shape(ndarray::ShapeError::from_kind(
...
fn run_encoder(
    ...
    check_feature_len(features, n_frames * n_mels)?;
    let input = TensorRef::from_array_view(([1_usize, n_frames, n_mels], features))?;
...
fn run_pooler(
    ...
    let input = TensorRef::from_array_view(([1_usize, c_mfa, n_frames], chanwise.as_slice()))?;
