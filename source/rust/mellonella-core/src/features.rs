...
pub struct Fbank {
    ...
    window: [f32; N_FFT],
    /// The `(N_STFT, N_MELS)` triangular filter weights, stored as one
    /// non-zero span per STFT bin rather than as a dense matrix.
    mel_spans: Box<[MelSpan]>,
}

/// One STFT bin's contribution to the mel bands.
///
/// The filterbank is triangular, so each STFT bin feeds only the two or
/// three mel bands whose triangles cover it — roughly 3 % of the
/// `N_MELS` columns. Iterating the dense row spent the other 97 % of
/// its multiply-adds on `power * 0.0`.
struct MelSpan {
    /// Index of the first mel band this bin feeds.
    first_mel: usize,
    /// Weights for `first_mel .. first_mel + weights.len()`.
    weights: Box<[f32]>,
}
...
impl Fbank {
    ...
    pub fn new(filterbank: &[f32]) -> Result<Self, FbankError> {
        ...
        // Collapse each row to its non-zero span. Trailing/leading zero
        // weights are dropped; interior weights are kept even if zero so
        // the span stays contiguous.
        let mel_spans = filterbank
            .chunks_exact(N_MELS)
            .map(|row| {
                let first_mel = row.iter().position(|w| *w != 0.0).unwrap_or(0);
                let end = row.iter().rposition(|w| *w != 0.0).map_or(0, |i| i + 1);
                let weights = if end > first_mel {
                    row[first_mel..end].to_vec().into_boxed_slice()
                } else {
                    Box::default()
                };
                MelSpan { first_mel, weights }
            })
            .collect::<Vec<_>>()
            .into_boxed_slice();
    ...
    pub fn compute(&mut self, audio: &[f32]) -> Vec<f32> {
        ...
        for f_idx in 0..n_frames {
            ...
            // skipping the filterbank's zeros.
            //
            // Bit-identical to the dense form: `b` is still walked in
            // ascending order, so the accumulation order into each
            // `row[m]` is unchanged, and every skipped term is
            // `power * 0.0`. `power` is a squared magnitude (finite and
            // non-negative) and the mel weights are non-negative, so
            // `row[m]` never reaches -0.0 and the skipped `+ 0.0` adds
            // are exact identities.
            let row = &mut out[f_idx * N_MELS..(f_idx + 1) * N_MELS];
            row.fill(0.0);
            for (span, &p) in self.mel_spans.iter().zip(power.iter()) {
                let target = &mut row[span.first_mel..span.first_mel + span.weights.len()];
                for (slot, &w) in target.iter_mut().zip(span.weights.iter()) {
                    *slot += p * w;
                }
            }
        }
