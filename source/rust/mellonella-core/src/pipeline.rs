...
pub fn process_offline(
    ...
    if let Some(stage) = components.tse.as_mut().filter(|_| tse_enabled) {
        ...
        head.audio = apply_envelope_dual_rate(
            ...
            &decisions,
            audio_sample_rate,
            // `StreamingState` has already scaled transition sample
            // indexes to the audio rate. Passing the 16 kHz decision
            // rate here scales them a second time (3× at 48 kHz),
            // which can move the first ON transition past the end of
            // the file and produce an all-zero TSE result.
            audio_sample_rate,
            *gate_cfg,
        )?;
        tse_applied = true;
