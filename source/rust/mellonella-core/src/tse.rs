...
impl TseSession {
    ...
    pub fn process_chunk_slice(
        ...
        // The released cumulative-normalisation graph returns NaN when
        // its very first chunk is exact digital silence (zero variance
        // with zeroed running statistics).  Do not advance state for a
        // silent chunk; silence is already the correct extracted output.
        // This also keeps a quiet pre-roll from poisoning every later
        // chunk in a long Discord session.
        if chunk.iter().all(|sample| sample.abs() < 1.0e-7) {
            return Ok(vec![0.0; chunk_len]);
        }
        ...
        // Fail closed for any numerical instability and make the next
        // voiced chunk recoverable.  Returning NaN to the audio sink is
        // both inaudible after PCM conversion and permanently poisons
        // the recurrent state.
        let finite = extracted.iter().all(|sample| sample.is_finite())
            && new_state
                .iter()
                .all(|tensor| tensor.iter().all(|value| value.is_finite()));
        if !finite {
            self.reset();
            return Ok(vec![0.0; chunk_len]);
        }
